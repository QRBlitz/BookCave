...
public class BookInfoOrder extends AppCompatActivity {
    ...
    Button bshow,buybook,rentbook,fav,unfav;
    String sn,sa, userid;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        unfav=findViewById(R.id.remFav);
        ...
        db.collection("Favorites").whereEqualTo("bookid", book_id).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                String id;
                for (QueryDocumentSnapshot document : task.getResult()) {
                    id = (String) document.get("bookid");
        ...
        fav.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                favorite.put("sellerbookid", sellerbookid);
                favorite.put("bookid", book_id);
                favorite.put("sellerid", sellerid);
                favorite.put("quantities", quantities);
                favorite.put("sellingprice", sprice);
                favorite.put("rentingprice", rprice);
                favorite.put("deliverycharges", dprice);
                favorite.put("title", book_title);
                favorite.put("author", book_author);
                favorite.put("description", book_desc);
                favorite.put("category", book_cat);
                favorite.put("thumbnail", image);
                favorite.put("preview", preview);

                favorite.put("favoriteid", uniqueid);
                favorite.put("customerid", userid);
                db.collection("Favorites").document(uniqueid).set(favorite).addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            Toast.makeText(BookInfoOrder.this, "Book added to favorite", Toast.LENGTH_SHORT).show();
                            unfav.setVisibility(View.VISIBLE);
                            fav.setVisibility(View.INVISIBLE);
                        }
                        else {
                            String error = Objects.requireNonNull(task.getException()).getMessage();
                            Toast.makeText(BookInfoOrder.this, "Error " + error, Toast.LENGTH_SHORT).show();
                        }
                    }
                });
            }
            ...
            public void onClick(View view) {
                db.collection("Favorites").whereEqualTo("bookid", book_id).get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        ...
                        for (QueryDocumentSnapshot document : task.getResult()) {
                            id = (String) document.get("bookid");
                            favid = (String) document.get("favoriteid");
                            if (id.equals(book_id)) {
                                db.collection("Favorites").document(favid).delete().addOnCompleteListener(new OnCompleteListener<Void>() {
                                    @Override
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            Toast.makeText(BookInfoOrder.this, "Book removed from favorite", Toast.LENGTH_SHORT).show();
